...
function resolveAppBuilderInstallUtil(): string {
  ...
  const appBuilderDir = readdirSync(bunModulesDir, { withFileTypes: true })
    ...
    .filter((candidate) => existsSync(resolve(candidate, 'package.json')))
    .sort()[0];
...
function mockedSpawnSync() {
  return { status: 0, stdout: Buffer.from(''), stderr: Buffer.from('') };
}

childProcess.execSync = mockedExecSync;
...
Module._load = function patchedLoad(request, parent, isMain) {
  ...
  if (request === 'child_process' || request === 'node:child_process') {
    return { ...childProcess, execSync: mockedExecSync, spawnSync: mockedSpawnSync };
  }
