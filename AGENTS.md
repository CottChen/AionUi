...
- Conversation capability pickers may add only backend-persisted, non-builtin MCP IDs. Extension-contributed or client-only MCP entries are not accepted by `/api/conversations/:id/capabilities`; show them only when already loaded, and keep add catalogs sourced from the backend MCP catalog.
- MCP catalog loading must tolerate malformed legacy `mcp.config` values and malformed rows from either settings or backend; one bad row must not clear the valid MCP list or hide the new-session selector.
...
- Do not identify a Web package by its repeated tarball filename alone. After every build and download, inspect `bundled-aioncore/<platform>/manifest.json` and verify the expected fork repository, AionCore run/commit, and `sourceType`; also verify the packaged binary contains the required `900021/900022/900023` migrations and that the tarball SHA-256 matches its companion checksum. This prevents installing an older artifact that still bundles official AionCore.
